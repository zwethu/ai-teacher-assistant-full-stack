...
from services.agent_engine_client import stream_agent_response
...
def _build_session_state(
    *,
    ...
    return {
        # Run telemetry keys
        "run_id": run_id,
        "session_id": chat_id,       # stable chat session — backend-supplied
        "chat_id": chat_id,
        "rtdb_run_path": rtdb_run_path,
...
async def _run_agent_background(
    *,
    ...
    try:
        ...
        add_message(batch_id, chat_id, "assistant", final_text, lecturer_id)
        ...
        # Persist a failure message so the chat isn't left blank
        error_text = f"(Agent run failed) {exc}"
        try:
            add_message(batch_id, chat_id, "assistant", error_text, lecturer_id)
            write_final_message(run_id, error_text)
        except Exception:
            pass
