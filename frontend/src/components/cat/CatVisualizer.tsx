...
export default function CatVisualizer({ mood, trustProgress, onPet }: CatVisualizerProps) {
  ...
  // Spawn visual cues according to mood changes
  useEffect(() => {
    if (mood === 'happy') {
      spawnParticle('💖');
      spawnParticle('✨');
      ...
      >
        ...
        <svg viewBox="0 0 200 240" className="w-full h-full overflow-visible">
          ...
          <g filter="url(#soft-shadow)">
            ...
            {/* TAIL */}
            <path
              d="M 130 190 Q 175 190, 165 130 T 180 80"
              fill="none"
              stroke="#fca5a5"
              strokeWidth="16"
              strokeLinecap="round"
              className={`origin-[130px_190px] ${
                mood === 'happy' || mood === 'purring' ? 'animate-tail-happy' : 'animate-tail-relaxed'
              }`}
            />
            <circle
              cx="180" cy="80" r="8" fill="#fff"
              className={`origin-[130px_190px] ${
                mood === 'happy' || mood === 'purring' ? 'animate-tail-happy' : 'animate-tail-relaxed'
              }`}
            />
