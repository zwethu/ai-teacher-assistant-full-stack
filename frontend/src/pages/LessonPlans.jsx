...
import { fromFirestore, toFirestore } from '../entity/LessonPlan.js'
import { generateLessonPlan } from '../services/agentService.js'
...
function formatContentForDisplay(content) {
  ...
  if (typeof content === 'string') {
    try {
      return formatContentForDisplay(JSON.parse(content))
    } catch {
      return content
    }
  }
...
export default function LessonPlans() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(
      ...
      (error) => {
        ...
        setToast({
          type: 'error',
          message: 'Could not load lesson plans. Check Firestore rules and indexes.',
        })
      },
    )
  ...
  function openGenerateModal() {
    if (!creditsLoading && credits <= 0) {
      showToast('error', 'You have no credits left. Cannot generate a lesson plan.')
      return
    }
  ...
  async function handleGenerate(e) {
    ...
    try {
      ...
      console.error(err)
      showToast('error', err.message || 'Failed to generate lesson plan.')
    } finally {
      setGenerating(false)
    }
  }
